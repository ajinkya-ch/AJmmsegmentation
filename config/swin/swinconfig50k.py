...
data = dict(
    ...
    train=dict(
        ...
        img_dir='/global/cfs/projectdirs/m636/avaylon/Fiber/Full/full/train/img',
        ann_dir='/global/cfs/projectdirs/m636/avaylon/Fiber/Full/full/train/ann',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations'),
            dict(type='RandomRotate', prob=0.5, degree=0.1),
        ...
        split='/global/homes/a/aj2000/vtfs/splits/train_exp1_50k.txt'),
    val=dict(
        type='FiberDataset',
        ...
        split='/global/homes/a/aj2000/vtfs/splits/val_exp1_10k.txt'),
    test=dict(
        type='FiberDataset',
...
runner = dict(type='IterBasedRunner', max_iters=62500)

checkpoint_config = dict(
    by_epoch=False, interval=62500, meta=dict(CLASSES=2, PALETTE=None))

evaluation = dict(interval=100, metric=['mIoU', 'mDice'], pre_eval=True)
work_dir = '/global/cfs/projectdirs/m636/AJ/p1_models/swin/swin_50000_bsize2'
seed = 0
gpu_ids = range(0, 4)